...
class Public {
    ...
    identify({
        ...
        if (post) {
            return this.publicRequest({ resource: IDENTIFY, params: { data: data, test: isTest }, post: post, headers: headers});
     ...
     * @param {boolean} isTest Should this be a test request.
     * @param {boolean} post Send as a POST request.
     * @param {dict} headers Specify optional per-request headers.
     * @returns {Promise.<number|KlaviyoAPIError>} 1 (pass) or 0 (fail) | Non-200 response received
     */
    track({
        event,
